...
* Added several type validation functions (`is_*_scalar`) for robust input 
  ...
  - `is_bool_scalar()` – check if a value is exactly `TRUE` or `FALSE`
...
* Added corresponding checker functions that raise an error if validation 
  fails, e.g.:

  - `check_bool_scalar(TRUE)` passes, `check_bool_scalar(NA)` throws a 
    `rlang_error`

  - `check_char_scalar("hello")` passes, `check_char_scalar(" ")` throws a 
    `rlang_error`

  - `check_numeric_scalar(0.5)` passes, `check_numeric_scalar(NA)` throws a 
    `rlang_error`

* Made all `is_*` and `check_*` functions consistent in naming and behavior.

* Added comprehensive testthat tests for all validation and check functions.
